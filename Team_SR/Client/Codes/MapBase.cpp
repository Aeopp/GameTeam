...
HRESULT CMapBase::RenderGameObject()
{
	...
	m_pDevice->SetTexture(_Effect.GetTexIdx("EnvironmentSampler"),_CubeTexture.get());
...
void CMapBase::Free()
{
	...
	if (_BarSubSetInfo)
	{
		for (auto& pBarInfo : *_BarSubSetInfo)
		{
			SafeRelease(pBarInfo.Diffuse);
			SafeRelease(pBarInfo.Specular);
			SafeRelease(pBarInfo.Normal);
			SafeRelease(pBarInfo.VtxBuf);
			SafeRelease(pBarInfo.Decl);
			pBarInfo.Release();
		}
		_BarSubSetInfo->clear();
	}
...
void CMapBase::CubeMapRender()
{
	...
	mat CubeWorld = MATH::WorldMatrix({ 3,3,3 }, { 0,0,0 },{ InvView._41,InvView._42,InvView._43});
...
void CMapBase::WallRender()
{
	...
	for (auto& RefInfo : *_WallSubSetInfo)
	{
		...
		{
			_Effect.SetVSConstantData(m_pDevice, "UVFlag", int32_t(2));
		}
...
void CMapBase::FloorRender()
{
	if (!_FloorSubSetInfo)return;
...
void CMapBase::BarRender()
{
	if (!_BarSubSetInfo)return;
	...
	for (auto& RefInfo : *_BarSubSetInfo)
	{
		...
		m_pDevice->SetStreamSource(0, RefInfo.VtxBuf, 0, sizeof(Vertex::Texture));
		m_pDevice->SetVertexDeclaration(RefInfo.Decl);
		if (RefInfo.MaterialInfo.TextureName == L"TEXTURE_GRAVEYARD_fence.png")
		{
			_Effect.SetVSConstantData(m_pDevice, "UVFlag", int32_t(2));
		}
		m_pDevice->DrawPrimitive(D3DPT_TRIANGLELIST, 0, RefInfo.TriangleCount);
...
void CMapBase::LoadBars(const std::wstring& FilePath)
{
		...
		std::vector<PlaneInfo> _PolygonPlanes;
		const std::wstring _MtlFileName = FilePath + L"Bars.mtl";
		std::wfstream _MtlStream(_MtlFileName);
		...
		std::wstring _ObjFileName = FilePath + L"Bars.obj";
		...
		while (_ObjStream)
		{
			...
			if (ToKen.empty() && !_Vertexs.empty())
			{
				...
				_Info.Decl = Vertex::Texture::GetVertexDecl(m_pDevice);
				_BarSubSetInfo->push_back(_Info);
			}
